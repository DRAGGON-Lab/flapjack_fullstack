...
The repository now includes a minimum hardening baseline:

- environment-driven backend config with fail-fast checks for non-development runs (`flapjack_api/flapjack_api/settings.py`)
- compose-managed environment loading via root `.env`
- health endpoint (`/api/healthz/`) and container health checks
- local backup/restore/migration scripts under `scripts/`
- `.env.example` files with placeholders only
- CI workflow for backend checks and frontend build

These changes are intended to preserve runtime behavior while reducing data-loss and secret-management risk.
